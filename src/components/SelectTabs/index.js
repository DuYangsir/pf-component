...
class SelectTabs extends Component{
  ...
  getChildContext(){
    return {onTag: this.tagClick, onStateShow: this.stateShow}
  }
  ...
  resetBtn = (data,type) => {
    let dataId = []
    type ? data.map(item => {dataId.push(item.id)}) : dataId.push(data.id)
    let dataList = this.state.dataList    
    dataList.map((item, index) => {
  ...
  render(){
    const selectClassName = classNames('pf-select-tabs', {
      [`pf-select-tabs-active`]: this.state.selectState || this.state.screenData.state
    })
    return (
      <div className={selectClassName}>
        ...
        (<div className="pf-content-wrap" onClick={this.closeSelect}>
          ...
          <SelectBox data={this.state.selectData} 
        ...
        }
        {
          ...
          (<div className="pf-content-wrap">
            <SelectListBox data={this.state.screenData.data}
                resetBtn={this.resetBtn}
                confirmBtn={this.confirmBtn}
            />
          </div>) : null
